...
if [[ "${NAMESPACE}" == "-" ]]; then
  gcloud container clusters update "${CLUSTER_NAME}" --project "${GCLOUD_PROJECT}" --region "${location}" --update-labels "owner=${OWNER_LABEL//\./_},cost-center=${COST_CENTER},purpose=${PURPOSE//\./_}" > /dev/null
