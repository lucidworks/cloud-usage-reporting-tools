...
if [[ "${NAMESPACE}" == "-" ]]; then
  gcloud container clusters update "${CLUSTER_NAME}" --project "${GCLOUD_PROJECT}" --region "${location}" --update-labels "owner=${OWNER_LABEL//\./_},cost-center=${COST_CENTER},purpose=${PURPOSE//\./_}"
...
if [[ "${COST_CENTER}" == 5??_* || "${COST_CENTER}" == 310_* ]] ; then
  ...
  if [[ "${NAMESPACE}" == "-" ]]; then
    gcloud container clusters update "${CLUSTER_NAME}" --project "${GCLOUD_PROJECT}" --region "${location}" --update-labels "end-date=${END_DATE}"
